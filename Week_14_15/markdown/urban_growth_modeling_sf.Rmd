...
path = 'https://services3.arcgis.com/i2dkYWmb4wHvYPda/ArcGIS/rest/services/region_county_clp/FeatureServer/0/query?where=1%3D1&objectIds=&time=&geometry=&geometryType=esriGeometryPolygon&inSR=&spatialRel=esriSpatialRelIntersects&resultType=none&distance=0.0&units=esriSRUnit_Foot&returnGeodetic=false&outFields=&returnGeometry=true&returnCentroid=false&featureEncoding=esriDefault&multipatchOption=xyFootprint&maxAllowableOffset=&geometryPrecision=&outSR=&defaultSR=&datumTransformation=&applyVCSProjection=false&returnIdsOnly=false&returnUniqueIdsOnly=false&returnCountOnly=false&returnExtentOnly=false&returnQueryGeometry=false&returnDistinctValues=false&cacheHint=false&orderByFields=&groupByFieldsForStatistics=&outStatistics=&having=&resultOffset=&resultRecordCount=&returnZ=false&returnM=false&returnExceededLimitFeatures=true&quantizationParameters=&sqlFormat=none&f=pgeojson&token='
...
```{r, warning = FALSE, message = FALSE}
sf.MSA_fishnet <- 
  st_make_grid(sf.MSA, 4000) %>%
  st_sf()

sf.MSA_fishnet <-
  sf.MSA_fishnet[sf.MSA,]
...
CLASS_DIR = 'data/StateClass_Projection_BAU_2001-2101'
POP_DIR = 'Population_Projection_BAU_2001-2101'

CLASS_FILE = '-SClass.tif'
POP_FILE = '-Sa-Population.tif'

class_year_path = function(year, type)
{
  type_folder = ifelse(
    type == 'class', CLASS_DIR, POP_DIR
  )
  type_file = ifelse(
    type == 'class', CLASS_FILE, POP_FILE
  )
  year = year %>% as.character()
  path = paste0('Sc1120-It0001-Ts', year, type_file) %>%
    paste(DIR, type_folder, ., sep='/')
  return (path)}

raster_download = function(year, type){
  class_year_path(year, type) %>%
    raster(.) %>%
    projectRaster(., crs = CA_proj$proj4string)
}

sf.lc.2001 = raster_download(2001, 'class')

sf.lc.2009 = raster_download(2009, 'class')

sf.lc.2019 = raster_download(2019, 'class')
...
sf.lc.00_19 = sf.lc.2019 - sf.lc.2001

# reclassMatrix <- 
#   matrix(c(
#     0,  0, 0, # between 0 to 12 goes to 1
#     1, 12, 1,
#     -1, 12, 1),
#   ncol=3, byrow=T)

# sf.lc.00_19 <- 
#   reclassify(sf.lc.00_19, reclassMatrix)

sf.lc.00_19[sf.lc.00_19 < 0] <- 1
sf.lc.00_19[sf.lc.00_19 > 0] <- 1

sf.lc.00_19[sf.lc.00_19 == 0] <- NA

names(sf.lc.00_19) <- "sf.lc.00_19"

sf.lc.00_19.pts <-
  rasterToPoints(sf.lc.00_19) %>%
  as.data.frame() %>%
  st_as_sf(coords = c("x", "y"), crs = CA_proj)

sf.lc.00_19 <- 
  aggregate(sf.lc.00_19.pts, sf.MSA_fishnet, sum) %>%
  mutate(sf.lc.00_19 = ifelse(is.na(sf.lc.00_19),0,1),
         sf.lc.00_19 = as.factor(sf.lc.00_19))

```



Now `reclassify` and convert all 0’s to `NA`. We apply a name to the raster with `names`. This is done to make it faster to join raster to the fishnet below. You can see the frequency table of values with `freq(sf.lc.00_19)`. There are 655 areas that changed from underdeveloped to developed in the data.
...
ggplot() +
  geom_sf(data=sf.MSA) +
  geom_point(data=sf.lc.00_19, 
             aes(x=xyC(sf.lc.00_19)$x, y=xyC(sf.lc.00_19)$y, colour=sf.lc.2001)) +
...
ggplot() +
  geom_sf(data=sf.MSA) +
  geom_tile(
    data=sf.lc.2001 %>% as.data.frame() #%>% filter(value > 0),
...
```{r, warning = FALSE, message = FALSE}
developed <- sf.lc.2001 == 02 | sf.lc.2001 == 03
forest <- sf.lc.2001 == 06
farm <- sf.lc.2001 == 08 | sf.lc.2001 == 12 
wetlands <- sf.lc.2001 == 09
otherUndeveloped <- sf.lc.2001 == 05 | sf.lc.2001 == 07  | sf.lc.2001 == 11 
water <- sf.lc.2001 == 01
...
aggregatedRasters <-
  aggregateRaster(theRasterList, sf.MSA_fishnet) %>%
...
grid.arrange(
ggplot() +
  geom_sf(data = sf.pop.00, aes(fill=factor(ntile(pop.2000,5))), colour=NA) +
  scale_fill_manual(values = palette5,
                    labels=quintileBreaks(sf.pop.00,"pop.2000"),
                   name="Quintile\nBreaks") +
  labs(title="Population, Houston MSA: 2000") +
  mapTheme,

ggplot() +
  geom_sf(data = sf.pop.10, aes(fill=factor(ntile(pop.2010,5))), colour=NA) +
  scale_fill_manual(values = palette5,
                    labels=quintileBreaks(sf.pop.10,"pop.2010"),
                   name="Quintile\nBreaks") +
...
The Census data `sf.pop.`, has a different spatial extent than `sf.MSA_fishnet`. Most notably, there are no vectors where water is present. To maintain the needed 18,025 grid cells units `(nrow(sf.MSA_fishnet))`, a a unique id is created, `fishnetID`. Then the area weighted interpolation is performed on `population` for the 2000 and 2010 layers. Finally, the results are joined back (`left_join`) to `sf.MSA_fishnet`. This approach maintains a consistent spatial extent.
...
_MF Note: This code chunk has notable differences from the original markdown. Most notably - the `rownames_to_column` function has been inserted into the dplyr chain as a way to assign out the fishnet cell ID from the sf.MSA_fishnet to the interpolated data. This allows it to be joined to the fishnet by it's unique ID. The `st_interpolate_aw` function removes the unique ID._
...
fishnetPopulation00 <-
  st_interpolate_aw(sf.pop.00["pop.2000"], sf.MSA_fishnet, extensive=TRUE) %>%
  as.data.frame(.) %>%
  rownames_to_column(var = "fishnetID") %>%
  left_join(sf.MSA_fishnet %>%
              mutate(fishnetID = as.character(fishnetID)),
            ., by=c("fishnetID"='fishnetID')) %>% 
  mutate(pop.2000 = replace_na(pop.2000,0)) %>%
  dplyr::select(pop.2000)

fishnetPopulation10 <-
  st_interpolate_aw(sf.pop.10["pop.2010"],sf.MSA_fishnet, extensive=TRUE) %>%
  as.data.frame(.) %>%
  rownames_to_column(var = "fishnetID") %>%
  left_join(sf.MSA_fishnet %>%
              mutate(fishnetID = as.character(fishnetID)),
            ., by=c("fishnetID"='fishnetID')) %>% 
  mutate(pop.2010 = replace_na(pop.2010,0)) %>%
  dplyr::select(pop.2010)

fishnetPopulation <- 
  cbind(fishnetPopulation00,fishnetPopulation10) %>%
  dplyr::select(pop.2000,pop.2010) %>%
  mutate(pop_Change = pop.2010 - pop.2000)
```
...
ggplot() +
  geom_sf(data=sf.pop.10, aes(fill=factor(ntile(pop.2010,5))),colour=NA) +
  scale_fill_manual(values = palette5,
                    labels=substr(quintileBreaks(sf.pop.10,"pop.2010"),1,4),
                   name="Quintile\nBreaks") +
  labs(title="Population, Houston MSA: 2010",
...
ggplot() +
  geom_sf(data=fishnetPopulation, aes(fill=factor(ntile(pop.2010,5))),colour=NA) +
  scale_fill_manual(values = palette5,
                   labels=substr(quintileBreaks(fishnetPopulation,"pop.2010"),1,4),
                   name="Quintile\nBreaks") +
  labs(title="Population, Houston MSA: 2010",
...
houstonHighways <-
  ...
  st_transform(st_crs(sf.MSA)) %>%
  st_intersection(sf.MSA)
```

```{r plot_highway, warning = FALSE, message= FALSE}
ggplot() +
  geom_point(data=fishnet, 
             aes(x=xyC(fishnet)[,1], y=xyC(fishnet)[,2],colour=sf.lc.2001),size=1.5) +
...
First, the highway layer is converted to raster. This is done by creating an `emptyRaster` of `NA` grid cells at the same spatial extent as `sf.lc.2001`. Then, `highway_raster` is created by converting `houstonHighways` to `sp` form and then to applying `rasterize`. The raster is then converted to points with `rasterToPoints` and `st_as_sf`, then `aggregate` is used to calculate mean distance by grid cell.
...
highwayPoints_fishnet <- 
  aggregate(highwayPoints, sf.MSA_fishnet, mean) %>%
...
The `tigris` package allows Texas county geometries to be downloaded. A spatial subset returns only the counties in the MSA. Note that the subset includes a negative 1000ft `st_buffer`. This is done because the spatial extent of `sf.MSA` intersects county boundaries that are actually outside of our study area. Buffering `sf.MSA` slightly limits the intersection range to only those counties in the study area.
...
studyAreaCounties <- 
  ...
  st_transform(st_crs(sf.MSA)) %>%
  dplyr::select(NAME) %>%
  .[st_buffer(sf.MSA,-1000), , op=st_intersects]
```
...
dat <- 
  ...
  dplyr::select(sf.lc.2001, developed, forest, farm, wetlands, otherUndeveloped, water,
                pop.2000, pop.2010, pop_Change, distance_highways,lagDevelopment) %>%
  st_join(studyAreaCounties) %>%
  mutate(developed10 = ifelse(sf.lc.2001 == 1 & developed == 1, 0, developed)) %>%
  filter(water == 0) 
```
...
```{r, warning = FALSE, message = FALSE}
dat %>%
  dplyr::select(distance_highways,lagDevelopment,sf.lc.2001) %>%
  gather(Variable, Value, -sf.lc.2001, -geometry) %>%
  ggplot(., aes(sf.lc.2001, Value, fill=sf.lc.2001)) + 
...
```{r, warning = FALSE, message = FALSE}
dat %>%
  dplyr::select(pop.2000,pop.2010,pop_Change,sf.lc.2001) %>%
  gather(Variable, Value, -sf.lc.2001, -geometry) %>%
  ggplot(., aes(sf.lc.2001, Value, fill=sf.lc.2001)) + 
    geom_bar(position = "dodge", stat = "summary", fun.y = "mean") +
    facet_wrap(~Variable) +
...
```{r, warning = FALSE, message = FALSE}
dat %>%
  dplyr::select(sf.lc.2001:otherUndeveloped,developed) %>%
  gather(Land_Cover_Type, Value, -sf.lc.2001, -geometry) %>%
   st_set_geometry(NULL) %>%
     group_by(sf.lc.2001, Land_Cover_Type) %>%
...
First, `dat` is split into training and test sets. Note how imbalanced the panel is with `table(datTrain$sf.lc.20011)`.
...
```{r, warning = FALSE, message = FALSE}
Model1 <- glm(sf.lc.2001 ~ wetlands + forest  + farm + otherUndeveloped, 
              family="binomial"(link="logit"), data = datTrain)

Model2 <- glm(sf.lc.2001 ~ wetlands + forest  + farm + otherUndeveloped + lagDevelopment, 
              family="binomial"(link="logit"), data = datTrain)
              
Model3 <- glm(sf.lc.2001 ~ wetlands + forest  + farm + otherUndeveloped + lagDevelopment + pop.2000, 
              family="binomial"(link="logit"), data = datTrain)          
              
Model4 <- glm(sf.lc.2001 ~ wetlands + forest  + farm + otherUndeveloped + lagDevelopment + pop.2000 + 
              pop.2010, 
              family="binomial"(link="logit"), data = datTrain)              
            
Model5 <- glm(sf.lc.2001 ~ wetlands + forest  + farm + otherUndeveloped + lagDevelopment + pop_Change, 
              family="binomial"(link="logit"), data = datTrain)              
              
Model6 <- glm(sf.lc.2001 ~ wetlands + forest  + farm + otherUndeveloped + lagDevelopment + pop_Change + 
              distance_highways, 
              family="binomial"(link="logit"), data = datTrain) 
```
...
predsForMap <-         
  dat %>%
    ...
    dplyr::select(sf.lc.2001,Threshold_5_Pct,Threshold_17_Pct) %>%
...
ConfusionMatrix.metrics <-
  dat %>%
    mutate(probs = predict(Model6, dat, type="response") ,
           Threshold_5_Pct = as.factor(ifelse(probs >= 0.05 ,1,0)),
           Threshold_17_Pct =  as.factor(ifelse(probs >= 0.17 ,1,0))) %>%
    mutate(TrueP_05 = ifelse(sf.lc.2001  == 1 & Threshold_5_Pct == 1, 1,0),
           TrueN_05 = ifelse(sf.lc.2001  == 0 & Threshold_5_Pct == 0, 1,0),
           TrueP_17 = ifelse(sf.lc.2001  == 1 & Threshold_17_Pct == 1, 1,0),
           TrueN_17 = ifelse(sf.lc.2001  == 0 & Threshold_17_Pct == 0, 1,0)) %>%
...
```{r, warning = FALSE, message = FALSE}
spatialCV_counties <-
  spatialCV(dat,"NAME","sf.lc.2001", Model6) %>%
  ...
  data.frame(
   ...
   left_join(
     dat %>%
       st_set_geometry(NULL) %>%
       group_by(NAME) %>%
       summarize(county_population_2010 = round(sum(pop.2010))))
...
dat_infill <-
  ...
  #calculate population change
    ...
    mutate(proportion_of_county_pop = pop.2010 / county_population_2010,
           pop_2020.infill = proportion_of_county_pop * county_projection_2020,
           pop_Change = round(pop_2020.infill - pop.2010),2) %>%
...
ggplot() +
  geom_sf(data=sf.MSA) +
  geom_raster(data = rbind(rast(sf.lc.2001) %>% mutate(label = "2001"),
...
```{r, warning = FALSE, message = FALSE}
developed_2001 <- 
  reclassify(sf.lc.2001,reclassMatrix)
